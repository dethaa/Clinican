...
Builder.load_file('shifa/signupdokter/signupdokter.kv')
...
class SignupdokterWindow(BoxLayout):
    ...
    def fetch_account_email(self, _email):
        val = (_email.rstrip())
        mycursor = mydb.cursor()
        query = "SELECT * FROM Dokter WHERE email='{0}'".format(val)
        print(query)
        mycursor.execute(query)
        myresult = mycursor.fetchall()
        print(myresult)
    ...
    def create_id(self):
        mycursor = mydb.cursor()
        query = "SELECT MAX(iddokter) FROM Dokter"
        mycursor.execute(query)
        myresult = mycursor.fetchall()
        print(myresult)
    ...
    def insert_account(self, val):
        mycursor = mydb.cursor()
        query = "INSERT INTO Dokter VALUES ({0}, '{1}', '{2}', '{3}', '{4}', '{5}')"\
...
class SignupdokterApp(App):

    def build(self):
        Window.size = (1280, 720)
        return SignupdokterWindow()
